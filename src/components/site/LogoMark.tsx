...
export function LogoMark({ size = 44, className = "" }: { size?: number; className?: string }) {
  return (
    <span
      className={`relative inline-flex items-center justify-center overflow-hidden rounded-2xl p-[5px] shadow-[0_8px_20px_-6px_rgba(0,0,0,0.6)] h-16 w-16 sm:h-[var(--logo-size)] sm:w-[var(--logo-size)] ${className}`}
