...
function SiteNav() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div className={`mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4 transition-all duration-500 lg:px-10 ${
        scrolled 
          ...
          {links.map((l) => (
            ...
            >
              {({ isActive }) => (
                <>
                  {l.label}
                  <span className={`absolute -bottom-1.5 left-0 h-px bg-[oklch(0.78_0.15_85)] transition-all duration-500 ease-out ${
      ...
      <div className="mx-auto grid max-w-[1400px] gap-12 px-6 pb-12 lg:px-10 md:grid-cols-12">
        ...
        <div className="md:col-span-3">
          ...
          <div className="mt-5 space-y-4 text-sm text-white/50">
            <a href={`tel:${HLB.phone1}`} className="gold-link flex items-center gap-3 transition-colors hover:text-white">
              <Phone className="h-4 w-4 text-white/30" /> {HLB.phone1}
            </a>
            <a href={`tel:${HLB.phone2}`} className="gold-link flex items-center gap-3 transition-colors hover:text-white">
              <Phone className="h-4 w-4 text-white/30" /> {HLB.phone2}
            </a>
            <a href={`mailto:${HLB.email}`} className="gold-link flex items-center gap-3 break-all transition-colors hover:text-white">
              <Mail className="h-4 w-4 shrink-0 text-white/30" /> {HLB.email}
            </a>
            <div className="flex gap-3 pt-1">
              <MapPin className="h-4 w-4 shrink-0 mt-0.5 text-white/30" />
              <span className="max-w-[200px]">{HLB.address}</span>
            </div>
          </div>
        </div>
      </div>
